...
│   ├── title-generator.ts      # TitleGenerator — fire-and-forget async title generation via a separate Bedrock model call
...
├── data/conversations/         # Persisted conversation JSON files (created at runtime)
...
- **Dependency injection**: `createRouter()` accepts `{ store, agent, config, library, titleGenerator }` — makes testing straightforward
